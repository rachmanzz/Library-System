...
<div class="ui five item labeled icon inverted menu">
  ...
  <a href="//localhost:8000/student/{{Auth::user()->id}}/profile"class="red item">
    <i class="user icon"></i>
    {{$student->pop()}} {{$lastName->pop()}}
  </a>
  <a href="//localhost:8000/student/{{Auth::user()->id}}/borrow" class="active red item">
    <i class="book icon"></i>
    Reserve Book
  </a>
  <a href="//localhost:8000/student/{{Auth::user()->id}}/transaction"class="blue item">
...
<div class="ui container">
  ...
  @endif
  <div class="ui fluid icon input"><input type="text" id="myInputTextField" placeholder="Search a Book"><i class="search icon"></i></div>
  <br/>
  <table id="example" class="ui celled table" cellspacing="0" width="100%">
        <thead>
            <tr>
              ...
              <th><center><i class="bar chart icon"></i>&nbsp;Stock</center></th>
              <th><center><i class="settings icon"></i>&nbsp;Action</center></th>
            </tr>
        </thead>
        <tfoot>
        ...
        <tbody>
 @foreach($status as $key => $book)
    <tr>
    @if($key >= 1 && $book->student_id > 1 && $book->student_id != null && $book->student_id != $id && ($status[$key]->book_id == $status[$key-1]->book_id || $status[$key]->book_id == $status[$key]->book_id || $status[$key]->book_id == $status[$key+1]->book_id) )
      echo {{$status[$key]->student_id}}
    @else
    ...
    @endif
      @if($book->student_id == $id && $book->status == 1)
       <td class="text-center"><center><button type"submit" class="ui blue button" id="button" disabled/><i class="info icon"></i>Requested</center></td>
      @elseif($book->student_id == $id && $book->status == 2)
      <td class="text-center"><center><button type"submit" class="ui red button" id="button" disabled /><i class="remove icon"></i>On Hand</center></td>
      @elseif(($book->student_id == $id && $book->status == 3) || ($book->status == null && $book->book_id == null && $book->student_id == null) )
      <td class="text-center"><center><button type"submit" class="ui green button" id="button"/><i class="plus icon"></i>Add to bag</center></td>
      @else
      <td class="text-center"><center><button type"submit" class="ui green button" id="button"/><i class="plus icon"></i>Add to bag</center></td>
      @endif
    </tr>
